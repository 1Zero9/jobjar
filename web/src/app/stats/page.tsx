...
export default async function StatsPage() {
  ...
  const maxPersonMonth = Math.max(...stats.byPerson.map((p) => p.month), 1);
      ...
      <main className="mx-auto flex w-full max-w-[32rem] flex-col gap-6">
        ...
        {stats.byPerson.length > 0 ? (
          <section className="stats-panel">
            ...
            <h2 className="recorded-title">Completions this month</h2>
              ...
              {stats.byPerson.map((person) => (
                <div key={person.name} className="stats-person-row">
                  <div className="stats-person-meta">
                    ...
                    <span className="stats-person-count">{person.month}</span>
                  </div>
                  <div className="stats-bar-track">
                    <div
                      className="stats-bar-fill"
                      style={{ width: `${Math.round((person.month / maxPersonMonth) * 100)}%` }}
                    />
                  </div>
                  {person.week > 0 ? (
              ...
              {stats.byRoom.map((room) => (
                <div key={room.name} className="stats-room-row">
                  ...
                  <div className="stats-room-counts">
                    ...
                    {room.doneThisMonth > 0 ? (
                      <span className="stats-room-done">{room.doneThisMonth} done</span>
                    ) : null}
                  </div>
                </div>
              ))}
